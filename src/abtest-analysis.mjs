import { runDatabricksStatement } from "./databricks-client.mjs";

function getSchemas(gameCode) {
...
function propZTest(n1, x1, n2, x2) {
...
function welchTTest(n1, mean1, std1, n2, mean2, std2) {
  if (!n1 || !n2) return null;
...
function bootstrapMeanDiff(ctrlVals, trtVals, B = 2000) {
...
// 세그먼트 분류: Non-payer / Non-whale / Whale
// Whale = 이전 30일 구매자 중 상위 5% (p95 이상)
// Non-whale = 구매자 하위 95%
// Non-payer = pre_revenue = 0
function percentileSegment(rows) {
...
// ids: number[] — 첫 번째 ID를 기준 레코드로 사용하고, 나머지 ID들과의 교집합 유저만 포함
function buildAssignedCTE(ids, schemas) {
...
function buildMetaQuery(id, schemas) {
  return `
SELECT
  ab.id,
  ab.type,
...
function buildGroupsQuery(ids, schemas) {
...
function buildRetentionQuery(ids, schemas, dtRange) {
  const assigned = buildAssignedCTE(ids, schemas);
  const dtStart = dtRange.start;
  const dtEnd   = dtRange.retEnd; // test_start + min(duration,360) + 2일 버퍼
...
function buildRevenueQuery(ids, schemas, analysisEndPst) {
  const assigned = buildAssignedCTE(ids, schemas);
  return `WITH ${assigned},
rev_raw AS (
  SELECT r.user_id,
...
function buildWagerQuery(ids, schemas, useEtl, dtRange, analysisEndPst) {
  const assigned = buildAssignedCTE(ids, schemas);
  if (useEtl) {
    return `WITH ${assigned},
per_user AS (
...
function buildRoocQuery(ids, schemas, useEtl, dtRange, analysisEndPst) {
  const assigned = buildAssignedCTE(ids, schemas);
  const dtStart = dtRange.start;
...
function buildDailySegCase(segBoundaries) {
...
function buildDailyRevenueQuery(ids, schemas, whaleThreshold) {
  const primaryId = ids[0];
  const intersectFilter = ids.length > 1
    ? `AND tag.user_id IN (
    SELECT user_id FROM ${schemas.db}.user_ab_test_tag
...
function buildDailyPreRevenueQuery(ids, schemas) {
  const assigned = buildAssignedCTE(ids, schemas);
  return `WITH ${assigned},
pre_rev_raw AS (
  SELECT r.user_id,
...
function buildProductTypeCase(p = 'p') {
  return `CASE
...
function buildProductDailyQuery(ids, schemas, gameCode, selectedTypes) {
  const assigned = buildAssignedCTE(ids, schemas);
...
function buildProductStatsQuery(ids, schemas, gameCode, selectedTypes) {
  const assigned = buildAssignedCTE(ids, schemas);
  const appName = getAppName(gameCode);
  const ptCase = buildProductTypeCase('p');
  const inList = selectedTypes.map(t => `'${t.replace(/'/g, "''")}'`).join(', ');
...
function buildPostRolloutQuery(ids, schemas, rolloutPst) {
  const assigned = buildAssignedCTE(ids, schemas);
  return `WITH ${assigned},
post_rev AS (
...
function buildPrePeriodQuery(ids, schemas) {
  const assigned = buildAssignedCTE(ids, schemas);
  // pre-period dt 범위: 할당일 기준 최대 30일 전 (UTC 버퍼 포함)
...
// Main entry point
export async function runAbtestAnalysis({ abTestId, gameCode, selectedProductTypes }) {
  const ids = String(abTestId || "").split(",").map(s => Number(s.trim())).filter(n => n && !isNaN(n))
    .sort((a, b) => a - b); // 오름차순: 가장 낮은 ID가 primary (assign_ts, test_start/end 기준)
  if (!ids.length) throw Object.assign(new Error("Invalid A/B Test ID."), { statusCode: 400 });
  const primaryId = ids[0];

  const schemas = getSchemas(gameCode);

  // Step 1: Meta (기준 ID 기준)
  ...
  if (!metaRows.length) throw Object.assign(new Error(`A/B Test ID ${primaryId} not found.`), { statusCode: 404 });
  const meta = metaRows[0];
  const duration = Number(meta.duration_days) || 0;
  const useEtl = duration > 60;

  // dt 파티션 필터용 UTC 날짜 계산 (event 테이블은 dt=UTC 기준 파티션)
  ...
  const dtRange = {
    ...
    retEnd: addDays(startUtc, Math.min(duration, 360) + 2) // retention 최대 D360 + 2일 버퍼
  };

  // analysisEndPst = test_end_pst (= rollout 기준)
  ...
  const controlName = groups.find(g => g.group_id == 0)?.group_name || "Control";
  const treatmentNames = groups.filter(g => g.group_id != 0).map(g => g.group_name);

  // SRM check — population_weight 기반 기대값 사용
  let srmWarning = null;
  if (groups.length >= 2) {
    // population_weight 파싱: "[75,25]" 또는 "[1,1]" 형태
    let weights = null;
    try {
    ...
    if (Array.isArray(weights) && weights.length === groups.length) {
      const weightTotal = weights.reduce((s, w) => s + Number(w), 0);
      // groups는 group_id 오름차순 정렬됨
      const sortedGroups = [...groups].sort((a, b) => Number(a.group_id) - Number(b.group_id));
      expectedArr = sortedGroups.map((_, i) => total * Number(weights[i]) / weightTotal);
    } else {
      // fallback: 균등 분배
    ...
    const sortedGroups = [...groups].sort((a, b) => Number(a.group_id) - Number(b.group_id));
    const chiSq = sortedGroups.reduce((s, g, i) => s + Math.pow(Number(g.n) - expectedArr[i], 2) / expectedArr[i], 0);
    ...
    if (chiSq > critical) {
      const p = Math.max(0, 1 - chiSq / (critical * 1.5)); // rough p-value approximation
  ...
  const allGroupNames = groups
    .slice().sort((a, b) => Number(a.group_id) - Number(b.group_id))
    .map(g => g.group_name);
  ...
  // Revenue raw → PCA+k-means segmentation
  ...
  const { segments: segmentAssignments, whaleThreshold } = revRawRows.length >= 3 ? percentileSegment(revRawRows) : { segments: revRawRows.map(() => "Non-payer"), whaleThreshold: 0 };
  const segmentedRows = revRawRows.map((r, i) => ({ ...r, segment: segmentAssignments[i] }));
  ...
  for (const name of allGroupNames) {
    revByGroup[name] = segmentedRows.filter(r => r.group_name === name).map(r => Number(r.revenue));
  }

  // CUPED: θ = Cov(Y_intest, Y_pre) / Var(Y_pre), pooled across all users
  const allN = segmentedRows.length;
  const meanPre    = segmentedRows.reduce((s, r) => s + Number(r.pre_revenue), 0) / (allN || 1);
  const meanIntest = segmentedRows.reduce((s, r) => s + Number(r.revenue), 0)     / (allN || 1);
  const covYX  = segmentedRows.reduce((s, r) => s + (Number(r.revenue) - meanIntest) * (Number(r.pre_revenue) - meanPre), 0) / (allN || 1);
  const varX   = segmentedRows.reduce((s, r) => s + (Number(r.pre_revenue) - meanPre) ** 2, 0) / (allN || 1);
  const varY   = segmentedRows.reduce((s, r) => s + (Number(r.revenue) - meanIntest) ** 2, 0) / (allN || 1);
  const theta  = varX > 1e-10 ? covYX / varX : 0;
  const rho    = (varX > 1e-10 && varY > 1e-10) ? covYX / Math.sqrt(varX * varY) : 0; // 상관계수
  const varianceReduction = +(rho ** 2 * 100).toFixed(1); // CUPED 분산 감소율 (%)
  // Adjusted per-user revenue: Y_adj_i = Y_intest_i - θ * (Y_pre_i - mean_pre)
  const cuperByGroup = {};
  for (const name of allGroupNames) {
    cuperByGroup[name] = segmentedRows
      .filter(r => r.group_name === name)
      .map(r => Number(r.revenue) - theta * (Number(r.pre_revenue) - meanPre));
  }

  // PCA 세그먼트 기반 pre_revenue min/max 및 SQL CASE WHEN용 근사 경계값 유도
  const segRevMap = {};
  for (const seg of segmentLabels) segRevMap[seg] = [];
  segmentedRows.forEach(r => { const v = Number(r.pre_revenue); if (v > 0 && segRevMap[r.segment]) segRevMap[r.segment].push(v); });
  ...
  const [postResult, dailyRevResult, dailyPreRevResult, productDailyResult, productStatsResult] = await Promise.all([
    ...
    selectedProductTypes?.length ? runDatabricksStatement({ statement: buildProductDailyQuery(ids, schemas, gameCode, selectedProductTypes), parameters: [], rowLimit: 200000 }) : Promise.resolve({ rows: [] }),
    selectedProductTypes?.length ? runDatabricksStatement({ statement: buildProductStatsQuery(ids, schemas, gameCode, selectedProductTypes), parameters: [], rowLimit: 500000 }) : Promise.resolve({ rows: [] }),
  ]);
  ...
  const pairRevenue = {}, pairRetention = {}, pairPrePeriod = {}, pairPostRollout = {}, pairSegmentEffects = {};
  ...
  for (let i = 0; i < allGroupNames.length; i++) {
    for (let j = i + 1; j < allGroupNames.length; j++) {
      ...
      pairRevenue[key] = {
        ...
        cuperMeans: { a: aCuper.length ? +(aCuper.reduce((s, v) => s + v, 0) / aCuper.length).toFixed(2) : 0,
      ...
      for (const seg of segmentOrder) {
        const aVals = segmentedRows.filter(r => r.group_name === a && r.segment === seg).map(r => Number(r.revenue));
        const bVals = segmentedRows.filter(r => r.group_name === b && r.segment === seg).map(r => Number(r.revenue));
  ...
  // 그룹별 세그먼트 revenue
  const groupSegmentRevenue = {};
  for (const name of allGroupNames) {
    groupSegmentRevenue[name] = {};
    for (const seg of segmentOrder) {
      const vals = segmentedRows.filter(r => r.group_name === name && r.segment === seg).map(r => Number(r.revenue));
  ...
  for (const name of allGroupNames) {
    ...
    groupSummary[name] = {
      n: Number(groups.find(g => g.group_name === name)?.n || 0),
  ...
  // Product breakdown: pair effects computed from aggregate stats
  const allProductTypes = [...new Set(productStatsRows.map(r => r.product_type))].filter(Boolean).sort();
  // 유저별 상품 매출 배열 구성 (purchasers only → 이후 0 패딩)
  const productUserRevenue = {}; // { group_name: { product_type: [rev, ...] } }
  ...
  const pairProductEffects = {};
  for (let i = 0; i < allGroupNames.length; i++) {
    for (let j = i + 1; j < allGroupNames.length; j++) {
      const a = allGroupNames[i], b = allGroupNames[j];
      const key = `${a}|${b}`;
      const nA = Number(groups.find(g => g.group_name === a)?.n || 0);
      const nB = Number(groups.find(g => g.group_name === b)?.n || 0);
